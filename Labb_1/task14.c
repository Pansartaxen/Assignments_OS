...
static void OGinit_matrix(void)
{
    int i, j;
    for (i = 0; i < SIZE; i++)
        for (j = 0; j < SIZE; j++) {
...
static void * init_matrix(void *row)
{
    int i = *(int*)row;
...
int main(int argc, char **argv)
{
    ...
    printf("Time used for creating matrix sequential: %.2lf seconds\n", dif);
    ...
    dif = (finish.tv_sec - start.tv_sec);
    dif += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
    printf("Time used for creating matrix multithread: %.2lf seconds\n", dif);
